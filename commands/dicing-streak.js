...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const winRate = totalGames > 0 ? (stats.gamesWon / totalGames * 100).toFixed(2) : 0;
      ...
      if (interaction.deferred || interaction.replied) {
        await interaction.editReply({ content: '❌ An error occurred while retrieving your dicing stats.' });
      } else {
        await interaction.reply({ content: '❌ An error occurred while retrieving your dicing stats.', ephemeral: true });
      }
    }
  },
  ...
  async run(message, args) {
    try {
      ...
      console.error('Error in dicing-streak command:', error);
      await message.reply('❌ An error occurred while retrieving your dicing stats.');
    }
  }
};