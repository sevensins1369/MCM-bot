  ...
  execute: withErrorHandling(async function (interaction) {
    ...
    try {
      ...
      throw new ValidationError(
        "Invalid minimum entry amount. Please enter a valid number."
      );
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `A new jackpot has been created! Enter for a chance to win the entire pot!`
      )
      .addFields(
        { name: "Currency", value: currency.toUpperCase(), inline: true },
        {
          name: "Minimum Entry",
          value: formatAmount(minEntryAmount),
          inline: true,
        },
        {
          name: "Drawing In",
          value: `<t:${Math.floor(drawTime.getTime() / 1000)}:R>`,
          inline: true,
        },
        {
          name: "Total Pot",
          value: `${formatAmount(jackpot.totalPot)} ${currency.toUpperCase()}`,
          inline: true,
        },
        ...
        {
          name: "\u200B",
          value: `Use \`/enter-jackpot ${jackpot.gameId.substring(
            0,
            8
          )} [amount]\` to enter!`,
        }
    ...
    logger.info(
      "CreateJackpot",
      `Jackpot ${jackpot.gameId} created by ${interaction.user.id}`,
      {
        currency,
        minEntryAmount,
    ...
    try {
      ...
      if (args.length < 3) {
        return message.reply(
          "❌ Usage: `!create-jackpot <currency> <min_entry> <duration_minutes>`"
        );
      }
      ...
      logger.info(
        "CreateJackpot",
        `Jackpot ${jackpot.gameId} created by ${message.author.id}`,
        {
          currency,
          minEntryAmount,
          drawTime: drawTime.toISOString(),
