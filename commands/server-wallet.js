...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName("view").setDescription("View the server wallet")
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("add")
  ...
  async execute(interaction) {
    try {
      ...
      if (subcommand === "view") {
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            {
              name: "07",
              value: formatAmount(serverWallet.osrs),
              inline: true,
            },
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `Added ${formatAmount(
              amount
            )} ${currency.toUpperCase()} to the server wallet.`
          )
          .addFields({
            name: "New Balance",
            value:
              formatAmount(serverWallet[currency]) +
              " " +
        ...
        if (BigInt(serverWallet[currency]) < amount) {
          throw new ValidationError(
            `Server wallet doesn't have enough ${currency.toUpperCase()}.`
          );
        }
        ...
        const embed = new EmbedBuilder()
          ...
          .setColor(0xdaa520)
          .setDescription(
            `Removed ${formatAmount(
              amount
            )} ${currency.toUpperCase()} from the server wallet.`
          )
          .addFields({
            name: "New Balance",
            value:
      ...
      if (error instanceof ValidationError) {
        ...
        console.error("Error in server-wallet command:", error);
        await interaction.reply({
          content: "❌ An error occurred while managing the server wallet.",
          ephemeral: true,
        });
      }
    }
  ...
  async run(message, args) {
    try {
      ...
      if (args.length < 1) {
        throw new ValidationError(
          "Usage: !server-wallet <view|add|remove> [amount] [currency]"
        );
      }
      ...
      if (subcommand === "view") {
        ...
        if (args.length < 3) {
          throw new ValidationError(
            "Usage: !server-wallet add <amount> <currency>"
          );
        }
        ...
        if (currency !== "osrs" && currency !== "rs3") {
          throw new ValidationError(
            'Invalid currency. Must be "osrs" or "rs3".'
          );
        }
        ...
        if (args.length < 3) {
          throw new ValidationError(
            "Usage: !server-wallet remove <amount> <currency>"
          );
        }
        ...
        throw new ValidationError(
          "Invalid subcommand. Valid subcommands: view, add, remove"
        );
      }
    } catch (error) {
      if (error instanceof ValidationError) {
        ...
        console.error("Error in server-wallet command:", error);
        await message.reply(
          "❌ An error occurred while managing the server wallet."
        );
      }
    }
  },
