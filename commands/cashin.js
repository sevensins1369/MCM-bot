...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const embed = new EmbedBuilder()
        .setColor(0x00ff00)
        .setTitle(`${EMOJIS.win} Cash In Request`)
      ...
      await interaction.reply({
        content:
          "✅ The staff has been notified of your cash in request. They will contact you shortly.",
        ephemeral: true,
      });
    } catch (error) {
      console.error("Error in /cashin command:", error);
      await interaction.reply({
  ...
  async handleButton(interaction) {
    try {
      ...
      const embed = EmbedBuilder.from(interaction.message.embeds[0])
        .setColor(0x3498db)
        .setTitle(`${EMOJIS.win} Cash In Request (Claimed)`)
  ...
  async run(message, args) {
    try {
      ...
      if (args.length < 1) {
        return message.reply(
          "❌ Please specify an amount. Usage: `!cashin <amount>`"
        );
      }
