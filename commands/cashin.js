...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("cashin")
    .setDescription("Notify staff that you would like to cash in.")
    .addStringOption((option) =>
      option
        .setName("amount")
        .setDescription("The amount you want to cash in (e.g., 100M, 1B)")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
  ...
  async execute(interaction) {
    try {
      ...
      logger.info(
        "CashInCommand",
        `Cash in request from ${interaction.user.tag}`,
        {
          amount,
          currency,
        }
      ...
      try {
        ...
        const embed = new EmbedBuilder()
          .setColor(0x000000)
          .setTitle(`${EMOJIS.bet} Cash In Request`)
          .setDescription(`${interaction.user.toString()} wants to cash in.`)
          .addFields(
        ...
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId(`claim_cashin_${interaction.user.id}`)
        ...
        activeRequests.set(`claim_cashin_${interaction.user.id}`, {
        ...
        logger.info(
          "CashInCommand",
          `Cash in request notification sent for ${interaction.user.tag}`,
          {
            messageId: message.id,
          }
        );
      ...
      logger.error("CashInCommand", `General error: ${error.message}`, error);
      await interaction.reply({
  ...
  async handleButton(interaction) {
    try {
      ...
      logger.info("CashInCommand", `Button clicked: ${interaction.customId}`, {
      ...
      if (!isCashier) {
        ...
        return interaction[replyMethod]({
          content:
            "❌ Only cashiers can claim requests. Contact staff if you wish to apply as a cashier.",
          ephemeral: true,
        });
      }
      ...
      try {
        ...
        const embed = EmbedBuilder.from(interaction.message.embeds[0])
          .setColor(0x000000)
          .setTitle(
            `${EMOJIS.bet} Cash In Request (Claimed) you may now dm the requester!`
          )
          .addFields({
            name: "Claimed By",
        ...
        try {
          await requestUser.send(
            `${
              EMOJIS.bet
            } Cashier ${interaction.user.toString()} will be ready to process your cash in request for **${
              request.amount
            } ${
              request.currency
        ...
        logger.info("CashInCommand", `Request successfully claimed`, {
          requestKey,
  ...
  async run(message, args) {
    try {
      ...
      if (args.length < 2) {
        return message.reply(
          "❌ Please specify an amount and currency. Usage: `!cashin <amount> <currency>`"
        );
      }
      ...
      logger.info(
        "CashInCommand",
        `Cash in request from ${message.author.tag}`,
        {
          amount,
          currency,
        }
      ...
      try {
        ...
        const embed = new EmbedBuilder()
          .setColor(0x137d02)
          .setTitle(`${EMOJIS.bet} Cash In Request`)
          .setDescription(`${message.author.toString()} wants to cash in.`)
          .addFields(
            { name: "Amount", value: amount, inline: true },
            { name: "Currency", value: currency, inline: true }
          )
        ...
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId(`claim_cashin_${message.author.id}`)
            .setLabel("Claim Request")
            .setStyle(ButtonStyle.Success)
        );
        ...
        // Store the request in the active requests map
        activeRequests.set(`claim_cashin_${message.author.id}`, {
        ...
        logger.info(
          "CashInCommand",
          `Cash in request notification sent for ${message.author.tag}`,
          {
            messageId: notificationMsg.id,
          }
        );
...
// This command allows users to notify staff that they want to cash in.
// It sends a message to a specified channel with the user's request and the amount they want to cash in.
