...
async function connectToDatabase() {
  try {
    ...
    for (const uri of connectionStrings) {
      ...
      try {
        logger.info('Database', `Attempting to connect to MongoDB with: ${uri.split('//')[1]}`);
    ...
    logger.warn('Database', 'Failed to connect to MongoDB. Falling back to file-based storage.');
    ensureDataDirectory();
