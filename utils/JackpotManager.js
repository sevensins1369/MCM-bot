...
const { ValidationError, DatabaseError, safeDbOperation } = require('../error-handler');
...
async function initializeJackpotManager() {
  try {
    ...
    logger.error('JackpotManager', 'Failed to initialize JackpotManager', error);
  }
}
...
async function loadActiveJackpots() {
  try {
    ...
    if (isUsingMongoDB()) {
      ...
      if (Jackpot) {
        const jackpots = await Jackpot.find({ status: { $in: ['active', 'drawing'] } });
        
        jackpots.forEach(jackpot => {
          activeJackpots.set(jackpot.gameId, jackpot.toObject ? jackpot.toObject() : jackpot);
        });
        
        logger.info('JackpotManager', `Loaded ${activeJackpots.size} active jackpots from database`);
      } else {
...
async function loadJackpotsFromFile() {
  try {
    ...
    try {
      ...
      logger.info('JackpotManager', `Created empty jackpots file at ${JACKPOT_FILE_PATH}`);
      return;
    }
    ...
    logger.info('JackpotManager', `Loaded ${activeJackpots.size} active jackpots from file`);
  } catch (error) {
  ...
  try {
    ...
    if (isUsingMongoDB()) {
      ...
      if (Jackpot) {
        ...
        logger.debug('JackpotManager', `Saved jackpot ${jackpot.gameId} to database`);
      } else {
        // Fallback to file-based storage
    ...
    logger.error('JackpotManager', `Failed to save jackpot ${jackpot.gameId}`, error);
...
async function saveJackpotToFile(jackpot) {
  try {
    ...
    logger.error('JackpotManager', `Failed to save jackpot ${jackpot.gameId} to file`, error);
  }
}
...
function scheduleJackpotDrawing(jackpot) {
  ...
  if (timeUntilDraw > 0) {
    ...
    logger.info('JackpotManager', `Scheduled jackpot ${jackpot.gameId} drawing for ${new Date(drawTime).toISOString()}`);
  } else {
    // If draw time is in the past, draw immediately
    logger.warn('JackpotManager', `Jackpot ${jackpot.gameId} draw time is in the past, drawing immediately`);
...
async function enterJackpot(gameId, userId, username, amount) {
  ...
  if (BigInt(amount) < BigInt(jackpot.minEntryAmount)) {
    throw new ValidationError(`Minimum entry amount is ${formatAmount(jackpot.minEntryAmount)} ${jackpot.currency.toUpperCase()}.`);
  }
  ...
  wallet[jackpot.currency] = (BigInt(wallet[jackpot.currency]) - BigInt(amount)).toString();
  await updateWallet(userId, wallet);
  
  // Calculate tickets (1 ticket per 1M)
  const tickets = Math.max(1, Math.floor(Number(BigInt(amount) / BigInt(1000000))));
...
async function drawJackpot(gameId) {
  ...
  try {
    ...
    if (jackpot.entries.length === 0) {
      logger.info('JackpotManager', `No entries for jackpot ${gameId}, cancelling`);
    ...
    winnerWallet[jackpot.currency] = (BigInt(winnerWallet[jackpot.currency]) + BigInt(jackpot.totalPot)).toString();
    ...
    throw new DatabaseError('Failed to draw jackpot winner. Rescheduled for 1 minute later.', error);
  }
}
...
function getActiveJackpots() {
  return Array.from(activeJackpots.values()).filter(j => j.status === 'active');
}
...
async function cancelJackpot(gameId, cancelledBy) {
  ...
  for (const entry of jackpot.entries) {
    try {
      ...
      wallet[jackpot.currency] = (BigInt(wallet[jackpot.currency]) + BigInt(entry.amount)).toString();
      await updateWallet(entry.userId, wallet);
      
      logger.info('JackpotManager', `Refunded ${entry.amount} ${jackpot.currency} to ${entry.userId} for cancelled jackpot ${gameId}`);
    } catch (error) {
      logger.error('JackpotManager', `Failed to refund ${entry.userId} for cancelled jackpot ${gameId}`, error);
    }
  }
  ...
  logger.info('JackpotManager', `Cancelled jackpot ${gameId} by ${cancelledBy}`);
...
function getUserWinChance(gameId, userId) {
  ...
  // Calculate total tickets
  const totalTickets = jackpot.entries.reduce((total, entry) => total + entry.tickets, 0);
